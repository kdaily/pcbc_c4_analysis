...
`r All.Files %>% select(file.name, file.id, file.fileType, file.fileSubType, file.Diffname_short)  %>% kable`
...
* Differentation Stage: EB
  * Fixed Effects: Gender, Reprogramming Gene Combination,Culture Conditions
  * Random Effects: Donor ID
...
```{r print.imp.genes.DE,cache=FALSE}
...
```
Number of differentially expressed genes at EB stage for adj.P.Val <= 0.05 and logFC >= 0.5 or <= -0.5
```{r print.imp.genes.EB,cache=FALSE}
...
kable(merge(tmp1,tmp2,by = 'Contrast Names', all = T), row.names = F)
```
Number of differentially expressed genes at ECTO stage for adj.P.Val <= 0.05 and logFC >= 0.5 or <= -0.5
```{r print.imp.genes.ECTO,cache=FALSE}
...
kable(merge(tmp1,tmp2,by = 'Contrast Names', all = T), row.names = F)
```
Number of differentially expressed genes at MESO5 stage for adj.P.Val <= 0.05 and logFC >= 0.5 or <= -0.5
```{r print.imp.genes.MESO5,cache=FALSE}
...
```{r print.imp.genes.SC, cache=FALSE}
