...
tmp.mirna <- d3.mirna %>% 
  ...
  rename(assay_direction=direction) %>% 
  unique
```
...
tmp2.mirna <- tmp.mirna %>% 
  left_join(tmp.mrna) %>%
  filter(!is.na(mrna_direction), !is.na(assay_direction))

foo.miRNA <- llply(orderedDiffState, 
                   function(x) filter(tmp2.mirna,
                                      (variable1 == x & mrna_direction == "down" & assay_direction == "up") | (variable2 == x & mrna_direction == "up" & assay_direction == "down")))

names(foo.miRNA) <- orderedDiffState
downreg.mrna.mirna <- ldply(foo.miRNA, I, .id="variable") %>% 
  unique %>% 
  mutate(assay="miRNA")
...
tmp.methylation <- d2.methylation %>% 
  ...
  rename(hgnc_symbol=nearestTx, assay_direction=direction) %>% 
  unique
```
...
foo.methyl <- llply(orderedDiffState, 
             function(x) filter(tmp.mrna.methylation, (variable1 == x & mrna_direction == "down" & assay_direction == "up") | (variable2 == x & mrna_direction == "up" & assay_direction == "down")))
...
```{r merge}
downreg <- rbind(downreg.mrna.mirna, downreg.mrna.methylation)
```

List of features passing down-regulated filters for each assay:

```{r mergetbl, echo=TRUE}
downreg %>% 
  dcast(variable2 + variable1 ~ assay, 
        value.var="feature", 
...
The results were manually inspected to identify enrichment groups specific to cluster 2, which appears to be activated in the stem cell state and is repressed over time.
