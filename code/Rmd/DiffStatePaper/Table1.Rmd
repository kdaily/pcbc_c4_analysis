...
```{r}
foo <- metadata %>% 
  select(dataType, Diffname_short, C4_Cell_Line_ID, Reprogramming_Vector_Type, 
         Reprogramming_Gene_Combination, Cell_Line_Type, Tissue_of_Origin, Gender) %>%
  unique %>%
  melt(id.vars=c("C4_Cell_Line_ID", "Diffname_short", "dataType"))

bar <- foo %>% 
  group_by(Diffname_short, dataType, variable, value) %>%
  tally
...
baz <- bar %>%
  ...
  dcast(variable + dataType ~ Diffname_short, value.var="s")
