...
# # knit this file to synapse, will create figs with provenance, and output html
# # with figures and captions
# knit2synapse::knitfile2synapse('code/Rmd/DiffStatePaper/SuppFig1.Rmd', owner='syn4909485', overwrite=TRUE)

# Make sure to commit and push first!
thisRepo <- getRepo('kdaily/pcbc_c4_analysis', ref="branch", refName='updatefig1')
...
diffStateColors <- c("SC"="orange", "EB"="red", "DE"="yellow",
                     "MESO-5"="green", "MESO-15"="blue", "MESO-30"="pink",
                     "ECTO"="black")
...
suppfig1.mRNA <- synStore(File('SuppFig1_mRNA.pdf', parentId='syn4909485'),
...
suppfig1.miRNA <- synStore(File('SuppFig1_miRNA.pdf', parentId='syn4909485'),
                           activityName="Create figure",
...
```{r plot_methylation, include=FALSE}
dfAnnotations <- metadataReordered %>% 
  rename(DiffState=Diffname_short) %>%
  select(DiffState)
...
ht <- Heatmap(countMat.methylation, name = "Beta", 
...
suppfig1.methylation <- synStore(File('SuppFig1_methylation.pdf', parentId='syn4909485'),
                                 activityName="Create figure",
                                 used=c('syn3355993', 'syn3219876', 'syn4609631', 'syn4483642'), 
                                 executed=thisScript)
...
The $$\(\log\)$$ CPM (counts per million) matrix for RNA-seq (syn4483934), $$\(\log_{10}\)$$ count matrix for miRNA-seq (syn3355993) and $$\(\beta\)$$ values for methylation (syn4527629) were used for visualization of global patterns across differentiation states. Only differentially expressed genes between any pair of differentiation states were used (mRNA: syn4484232; miRNA: syn4609631; methylation: syn4527629; mRNA log fold change $$\(>`r lfc.threshold.rna`\)$$ and adjusted $$\(p\)$$-value $$\(<`r p.threshold.rna`\)$$; $$\(n=`r nrow(countMat.rna)`\)$$ genes; miRNA log fold change $$\(>`r lfc.threshold.mirna`\)$$ and adjusted $$\(p\)$$-value $$\(<`r p.threshold.mirna`\)$$; $$\(n=`r nrow(countMat.miRNA)`\)$$ miRNAs; methylation log fold change $$\(>`r lfc.threshold.methylation`\)$$ and adjusted $$\(p\)$$-value $$\(<`r p.threshold.methylation`\)$$; $$\(n=`r nrow(countMat.methylation)`\)$$ probes). For each assay, features (e.g., genes, miRNA's and methylation probes, respectively) were clustered using hierarchical clustering (Euclidean distance and average agglomeration method). The samples were clustered using hierarchical clustering (Euclidean distance and average agglomeration method) within each differentiation state. The differentiation states were then ordered temporally with respect to the level of differentiation.