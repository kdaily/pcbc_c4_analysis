...
files <- files[files$entity.id!='syn2654330',]  #Remove array description file
#files <- lapply(files$entity.id, function (x) synGet(x, downloadLocation=getwd()))
```

Load the methylation data an add the metadata
```{r readChunk, cache=TRUE, dependson="downloadChunk"}
RGSet <- read.450k.exp(base = getwd())
